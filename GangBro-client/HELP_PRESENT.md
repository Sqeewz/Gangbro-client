...
3. [📜 ระบบ Missions - Mission Hub](#-ระบบ-missions---mission-hub)
    - [🤝 เข้าร่วม Mission (JOIN) และถอนตัว (LEAVE)](#-เข้าร่วม-mission-join-และถอนตัว-leave)
...
```typescript
// ในหน้า Mission Lobby เมื่อเรากดปุ่ม JOIN
async onJoin(missionId: number) {
  // 1. ระบบจะเด้ง Confirm Dialog เพื่อความชัวร์ (Enlist Request)
  ...
  ref.afterClosed().subscribe(async (res) => {
    if (res) {
      // 2. เรียก API เพื่อส่งเราเข้าไปอยู่ใน Roster (บัญชีรายชื่อ) ของภารกิจ
      await this._mission.join(missionId)  
      
      // 3. เมื่อ Join สำเร็จ ระบบจะพาเราวาร์ปไปยังหน้า About Mission (ห้องสื่อสาร) ทันที!
      this._router.navigate(['/about-mission', missionId]);
    }
  });
}
...
**💡 อธิบายแบบเห็นภาพ:** การ JOIN เหมือนการที่คุณเดินไปลงบันทึกชื่อในบัญชีรายชื่อของหน่วยรบ (Roster) เมื่อลงชื่อเสร็จแล้ว ระบบจะพาคุณเข้าสู่ศูนย์บัญชาการ (About Mission) เพื่อคุย Chat วางแผนกับเพื่อนร่วมทีมได้แบบ Real-time เลย! 🚀

#### **4. ออกจาก Mission (Leave)**
```typescript
async onLeave(id: number) {
  // ถามให้แน่ใจก่อนทิ้งทีม! (ใช้วิธีเรียก helper confirmAndExecute)
  const success = await confirmAndExecute(
    this._dialog, 
    MISSION_MESSAGES.ABANDON, 
    () => this._missionService.leave(id)
  );
  
  if (success) {
    this.loadMissions(); // อัพเดทรายการ Missions ของเรา
  }
}
```

**💡 อธิบาย:** การกด Leave จะเรียก API เพื่อถอนตัวออกจาก Crew ของ Mission (มีผลเฉพาะตอนที่ Mission ยังเป็น `Open`) เพื่อเปิดทางให้คนอื่นเข้ามาแทน หรือถ้าเราเปลี่ยนใจไม่อยากทำแล้ว

#### **5. Auto-Polling (อัพเดทอัตโนมัติ)**
```typescript
